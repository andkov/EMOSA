...
model<-"OriginalContagionGauss"
distribution<-"Gauss"

pathDirectory <- file.path(getwd())
# pathModel <- file.path(pathDirectory, paste(model),"OriginalContagionBeta.bugs")
# pathData <- file.path(pathDirectory, "Data/SummaryBirthYearByTime.csv")
# pathOutChains <- file.path(pathDirectory, paste0("Data/ChainsContagion", cohortYear, ".csv"))
...
{ #This bracket permits the 'else' clause (because it's located on the top layer of the code.)
  if( basename(getwd()) == "EMOSA" ) {#This clause executes when run from the *.R file.
    pathModel <- file.path(getwd(),paste(model),"OriginalContagionGauss.bugs")
    ...
    pathModel <- file.path(dirname(getwd()), paste(model), "OriginalContagionGauss.bugs")
...
effSize
sink()
# THis file to be read and processed by "OriginalContagionGauss_resultsIn.xlsx"  
# located in the same folder "GitHub/EMOSA/OriginalContagion"
