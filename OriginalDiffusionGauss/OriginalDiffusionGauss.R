...
model<-"OriginalDiffusionGauss"
distribution<-"Gauss"

pathDirectory <- file.path(getwd())
# pathModel <- file.path(pathDirectory, paste(model),"OriginalDiffusionBeta.bugs")
...
{ #This bracket permits the 'else' clause (because it's located on the top layer of the code.)
  if( basename(getwd()) == "EMOSA" ) {#This clause executes when run from the *.R file.
    pathModel <- file.path(getwd(),paste(model),"OriginalDiffusionGauss.bugs")
    ...
    pathModel <- file.path(dirname(getwd()), paste(model), "OriginalDiffusionGauss.bugs")
...
effSize
sink()
# THis file to be read and processed by "OriginalDiffusionGauss_resultsIn.xlsx"  
# located in the same folder "GitHub/EMOSA/OriginalDiffusion"
