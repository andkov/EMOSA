...
cohortYear <- 1981 #1980, 1981, 1982, 1983, 1984
...
{ #This bracket permits the 'else' clause (because it's located on the top layer of the code.)
  if( basename(getwd()) == "EMOSA" ) {#This clause executes when run from the *.R file.
    pathModel <- file.path(getwd(),paste(model),"AlternativeContagionBeta.bugs")
    ...
    pathModel <- file.path(dirname(getwd()), paste(model), "AlternativeContagionBeta.bugs")
