...
model<-"ScaledContagion"
distribution<-"Gauss"

pathDirectory <- file.path(getwd())
# pathModel <- file.path(pathDirectory, paste(model),"ScaledContagionBeta.bugs")
...
{ #This bracket permits the 'else' clause (because it's located on the top layer of the code.)
  if( basename(getwd()) == "EMOSA" ) {#This clause executes when run from the *.R file.
    pathModel <- file.path(getwd(),paste(model),"ScaledContagionGauss.bugs")
    ...
    pathModel <- file.path(dirname(getwd()), paste(model), "ScaledContagionGauss.bugs")
...
effSize
sink()
# THis file to be read and processed by "examining the results of the Scaled Hibrid from WINBUGS.xlsx"  
# located in the same folder "GitHub/EMOSA/ScaledHybrid"
