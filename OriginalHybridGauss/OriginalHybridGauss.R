...
model<-"OriginalHybridGauss"
distribution<-"Gauss"

pathDirectory <- file.path(getwd())
# pathModel <- file.path(pathDirectory, paste(model),"OriginalHybridBeta.bugs")
...
{ #This bracket permits the 'else' clause (because it's located on the top layer of the code.)
  if( basename(getwd()) == "EMOSA" ) {#This clause executes when run from the *.R file.
    pathModel <- file.path(getwd(),paste(model),"OriginalHybridGauss.bugs")
    ...
    pathModel <- file.path(dirname(getwd()), paste(model), "OriginalHybridGauss.bugs")
...
# For Contagion or Diffusion or "brute-force Hybrid"
parametersToTrack <- c("Tgi", "Tga", "Tig", "Tia", "Tag", "Tai", 
                       "sigmaG", "sigmaA")
...
effSize
sink()
# THis file to be read and processed by "OriginalHybridGauss_resultsIn.xlsx"  
# located in the same folder "GitHub/EMOSA/OriginalHybrid"
