...
p<-ggplot(dsFORp, aes(x=cohort))+
  ...
  annotate("text", x=1983,y=.27, label ="contagion", color="goldenrod1", size=7)+
  annotate("text", x=1983,y=.22, label ="diffusion", color="royalblue2", size=7)+
  annotate("text", x=1983,y=.17, label ="hybrid", color="violet", size=7)+
  scale_y_continuous("Misfit: SS (lower=better)")
p 
plast<-p
pathFileOut<-file.path(getwd(),"Results","Sketches3","Fit - 1x1 -Prevalences.png") 
...
p 
plast<-p
pathFileOut<-file.path(getwd(),"Results","Sketches3","Fit - 1x1 -Transitions.png") 
png (filename = pathFileOut ,
     width = 300, height = 300 , units = "px")
plot(plast)
dev.off()
...
p   
plast<-p
pathFileOut<-file.path(getwd(),"Results","Sketches3","Fit - 1x3 -Prevalences.png") 
png (filename = pathFileOut ,
...
p 
plast<-p
pathFileOut<-file.path(getwd(),"Results","Sketches3","Fit - 3x3 -Transitions.png") 
png (filename = pathFileOut ,
